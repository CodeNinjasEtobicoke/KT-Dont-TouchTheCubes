...
public class PlayerControls : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        rb.velocity += transform.rotation * (Vector3.right * Input.GetAxisRaw("Horizantal") * 10f * Time.deltaTime);
        rb.velocity += transform.rotation * (Vector3.right * Input.GetAxisRaw("Horizantal") * 10f * Time.deltaTime);
        transform.position = new Vector3(Mathf.Clamp(transform.position.x, -30f, 30f),
            transform.position.y, Mathf.Clamp(transform.position.z, -30f, 30));       
